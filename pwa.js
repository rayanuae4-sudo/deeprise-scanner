...
async function registerWorker(){if(!('serviceWorker'in navigator))return;try{const reg=await navigator.serviceWorker.register('./sw.js?v=network-v17-0-0',{updateViaCache:'none'});await reg.update();const marker='deeprise_sw_cleanup_1700';if(!sessionStorage.getItem(marker)&&'caches'in window){sessionStorage.setItem(marker,'1');const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('deeprise-pwa-')&&k!=='deeprise-pwa-network-v17-0-0').map(k=>caches.delete(k)))} }catch(_e){}}
...
function notificationCenterLayer(){load('dr-notification-center-v167-loader','./deeprise-notification-center-v167.js?v=1680')}
function experienceLayer(){load('dr-experience-v169-loader','./deeprise-experience-v169.js?v=1690')}
...
brand();languagePerformancePatch();fastSearchLayer();notificationCenterLayer();experienceLayer();
