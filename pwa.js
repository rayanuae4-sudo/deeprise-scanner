...
function languagePerformancePatch(){try{if(typeof languageObserver!=='undefined')languageObserver.disconnect()}catch(e){}let tm=null;const schedule=()=>{clearTimeout(tm);tm=setTimeout(()=>{try{if(typeof applyLanguage==='function')applyLanguage()}catch(e){}},350)};const opt={childList:true,subtree:true};['results','top3','signalResultsPanel'].forEach(id=>{const el=document.getElementById(id);if(el)try{new MutationObserver(schedule).observe(el,opt)}catch(e){}});window.DeepRiseLanguageRefresh=schedule}
if('serviceWorker' in navigator)addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=stable-v15-9',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{}));
...
function load(id,src){if(document.getElementById(id))return;let s=document.createElement('script');s.id=id;s.src=src;s.defer=true;document.body.appendChild(s)}
...
addEventListener('load',()=>{brand();languagePerformancePatch();setTimeout(button,1800);setTimeout(nextCandleLayer,100);setTimeout(forecastSafetyLayer,140);setTimeout(signalEngineLayer,180);setTimeout(uiLayer,220);setTimeout(whaleLayer,260);setTimeout(whaleFlowLayer,320);setTimeout(publicLiquidityLayer,360);setTimeout(tradeMonitorLayer,400);setTimeout(tradeZonesLayer,450);setTimeout(moveTimingLayer,500);setTimeout(entryRankingLayer,580);setTimeout(integrityLayer,680);setTimeout(signalCardUiLayer,760);setTimeout(whaleInlineLayer,840);setTimeout(earlyBreakoutLayer,920);setTimeout(brand,1600)});
setTimeout(brand,250);setTimeout(languagePerformancePatch,320);setTimeout(nextCandleLayer,700);setTimeout(forecastSafetyLayer,760);setTimeout(signalEngineLayer,800);setTimeout(uiLayer,840);setTimeout(whaleLayer,880);setTimeout(whaleFlowLayer,940);setTimeout(publicLiquidityLayer,980);setTimeout(tradeMonitorLayer,1020);setTimeout(tradeZonesLayer,1070);setTimeout(moveTimingLayer,1120);setTimeout(entryRankingLayer,1200);setTimeout(integrityLayer,1300);setTimeout(signalCardUiLayer,1380);setTimeout(whaleInlineLayer,1460);setTimeout(earlyBreakoutLayer,1540);setTimeout(brand,1800);
})();