/* DeepRise V15.4 — conservative entry decision gate. READY is the only executable LONG/SHORT state. */
(()=>{'use strict';
...
const TXT={
 en:{signal:'SIGNAL',watch:'WATCH',pump:'PUMP',ready:'ENTRY READY',confirmed:'CONFIRMED — WAIT TRIGGER',forming:'FORMING',wait:'WAIT'},
 ar:{signal:'سيجنال',watch:'مراقبة',pump:'بامب',ready:'الدخول جاهز',confirmed:'مؤكد — انتظر المحفز',forming:'قيد التكوين',wait:'انتظار'},
 ru:{signal:'СИГНАЛ',watch:'WATCH',pump:'PUMP',ready:'ВХОД ГОТОВ',confirmed:'ПОДТВЕРЖДЕНО — ЖДАТЬ',forming:'FORMING',wait:'WAIT'}
};
...
function hiddenGate(x,side){
 const p=x?.proSignal||{},confidence=n(p.confidence,x?.score),rank=n(p.rank,confidence),rsi=n(x?.rsi,50),expected=n(p.expectedMovePct,0),detailed=!!p.detailed,riskCount=oppositeRiskCount(x,side),reasons=[];
 const checks={
  forecastAlignment:!detailed||p.multiTfAligned===true,
  signalStrength:confidence>=74&&rank>=70,
  expectedDirection:!p.direction||((side==='LONG'&&expected>0)||(side==='SHORT'&&expected<0)),
 ...
 return{ok:Object.values(checks).every(Boolean),checks,reasons,confidence,rank,riskCount,detailed};
}
function quality(x){
 const side=rawSide(x),long=side==='LONG',short=side==='SHORT';
 if(!x||(!long&&!short))return{side:'WAIT',count:0,state:'WAIT',items:[],entryReady:false,hardAligned:false,hidden:{ok:false,reasons:['no directional edge']}};
 ...
 const count=items.filter(v=>v[1]).length,hardAligned=!!(items[0][1]&&items[1][1]),hidden=hiddenGate(x,side);
 let state='WAIT';
 if(count===6&&hardAligned&&hidden.ok)state='READY';
 ...
 return{side,count,state,items,entryReady:state==='READY',hardAligned,confidence,rank,hidden};
}
...
function applyGate(){
 installQuality();
 for(const x of data()){
  const q=quality(x);x.v13Quality=q;x.entryDecision={state:q.state,side:q.side,count:q.count,confidence:q.confidence||0,rank:q.rank||0,hidden:q.hidden,entryAllowed:q.entryReady,updated:Date.now()};
  if((q.side==='LONG'||q.side==='SHORT')&&q.state==='READY')x.direction=q.side;else x.direction='WAIT';
 }
}
function strength(x,f){const q=quality(x),p=n(x?.proSignal?.rank,x?.score),c=n(x?.proSignal?.confidence,x?.score),pump=n(x?.pump?.score);if(f==='pump')return pump*2+p*.35+q.count*2;if(f==='wait')return q.count*24+p+c*.2+(q.state==='CONFIRMED'?24:q.state==='FORMING'?14:0);const bonus=q.state==='READY'?90:q.state==='CONFIRMED'?38:q.state==='FORMING'?12:0;return bonus+p+c*.5+q.count*7+n(x?.volumeRatio)*2-(q.hidden?.riskCount||0)*12}
