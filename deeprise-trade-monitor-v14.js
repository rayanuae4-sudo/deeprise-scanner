...
const copy={
 en:{enter:'ENTER TRADE',active:'ACTIVE MONITORING',analyze:'ANALYZE & ENTER',noEntry:'No confirmed LONG/SHORT entry yet.',entered:'Trade entered — intensive monitoring is active.',stopped:'Trade monitoring stopped.',title:'🎯 Active Trade Monitor',live:'LIVE PRICE',deep:'20s DEEP ANALYSIS',entry:'Entry',stop:'Stop',tp1:'TP1',tp2:'TP2',score:'Score',remove:'Stop monitoring',note:'Live price alerts run while the app is active. Browser notifications depend on device permission.',rev:'Reversal warning',flip:'Direction reversal',weak:'Setup weakening',sl:'Stop level reached',t1:'TP1 reached',t2:'TP2 reached'},
 ar:{enter:'دخول الصفقة',active:'مراقبة مكثفة نشطة',analyze:'حلّل ثم ادخل',noEntry:'لا توجد إشارة دخول LONG/SHORT مؤكدة الآن.',entered:'تم تسجيل دخول الصفقة وتفعيل المراقبة المكثفة.',stopped:'تم إيقاف مراقبة الصفقة.',title:'🎯 مراقبة الصفقات النشطة',live:'السعر اللحظي',deep:'تحليل عميق كل 20ث',entry:'الدخول',stop:'الوقف',tp1:'الهدف 1',tp2:'الهدف 2',score:'السكور',remove:'إيقاف المراقبة',note:'تنبيهات السعر اللحظية تعمل أثناء نشاط التطبيق. إشعارات الجهاز تعتمد على سماح المتصفح.',rev:'تحذير انعكاس',flip:'انعكاس الاتجاه',weak:'ضعف الصفقة',sl:'وصل السعر إلى الوقف',t1:'تحقق الهدف الأول',t2:'تحقق الهدف الثاني'},
 ru:{enter:'ВОЙТИ В СДЕЛКУ',active:'УСИЛЕННЫЙ МОНИТОРИНГ',analyze:'АНАЛИЗ + ВХОД',noEntry:'Подтверждённого LONG/SHORT входа пока нет.',entered:'Сделка сохранена — усиленный мониторинг включён.',stopped:'Мониторинг сделки остановлен.',title:'🎯 Активные сделки',live:'LIVE ЦЕНА',deep:'ГЛУБОКИЙ АНАЛИЗ 20с',entry:'Вход',stop:'Стоп',tp1:'TP1',tp2:'TP2',score:'Score',remove:'Остановить мониторинг',note:'Live-оповещения работают пока приложение активно. Системные уведомления зависят от разрешения устройства.',rev:'Риск разворота',flip:'Разворот направления',weak:'Ослабление сетапа',sl:'Достигнут стоп',t1:'Достигнут TP1',t2:'Достигнут TP2'}};
...
const sel=s=>{try{return window.CSS&&CSS.escape?CSS.escape(String(s)):String(s).replace(/[^A-Za-z0-9_-]/g,'\\$&')}catch(e){return String(s)}};
...
function annotateCards(){
 document.querySelectorAll('#results .card,.top-grid .pick').forEach(card=>{let s=card.dataset.symbol||card.dataset.drSymbol;if(!s){const txt=card.querySelector('.sym')?.textContent||'';const base=txt.split('/')[0].trim().replace(/[^A-Z0-9]/gi,'').toUpperCase();if(base)s=base+'USDT'}if(!s)return;card.dataset.drSymbol=s;
  ...
  btn.classList.toggle('active',active);const label=active?'✓ '+t('active'):(a?(a.direction==='WAIT'?t('analyze'):t('enter')):t('analyze'));if(btn.textContent!==label)btn.textContent=label;
 })
}
function queueInject(){if(state.injectQueued)return;state.injectQueued=true;requestAnimationFrame(()=>{state.injectQueued=false;annotateCards()})}
...
function renderPanel(){
 ...
 const html=panelHtml(entries);if(p._drHtml!==html){p._drHtml=html;p.innerHTML=html}
}
...
function priceCheck(symbol,price){const tr=state.trades[symbol];if(!tr)return;tr.livePrice=price;
 if(tr.side==='LONG'){
  ...
  if(price>=tr.tp2)fire(tr,'tp2',t('t2'),tradeBody(tr,price),'green',365*24*3600*1000);else if(price>=tr.tp1)fire(tr,'tp1',t('t1'),tradeBody(tr,price),'green',365*24*3600*1000)
 }else{
  if(price>=tr.sl)fire(tr,'stop',t('sl'),tradeBody(tr,price),'red',365*24*3600*1000);
  if(price<=tr.tp2)fire(tr,'tp2',t('t2'),tradeBody(tr,price),'green',365*24*3600*1000);else if(price<=tr.tp1)fire(tr,'tp1',t('t1'),tradeBody(tr,price),'green',365*24*3600*1000)
 }
 const now=Date.now();if(now-state.lastUi>700){state.lastUi=now;renderPanel()}
}
...
async function deepCheckOne(symbol){const tr=state.trades[symbol];if(!tr)return;let a=null;try{a=await window.DeepRiseBinanceUniverse?.analyze?.(symbol,true)}catch(e){}if(!a)return;const r=reversalReason(a,tr);if(r)fire(tr,r.key,r.title,r.body,r.kind,r.key==='flip'?5*60*1000:ALERT_COOLDOWN);tr.lastScore=+a.score||tr.lastScore;tr.lastDirection=a.direction;save();renderPanel();queueInject()}
...
function restoreAnchor(){
 if(state.restoreQueued||Date.now()-state.userActionAt<850||!state.anchor)return;state.restoreQueued=true;requestAnimationFrame(()=>{state.restoreQueued=false;if(Date.now()-state.userActionAt<850)return;const s=sel(state.anchor.symbol),el=document.querySelector(`#results [data-dr-symbol="${s}"],#results [data-symbol="${s}"]`);if(!el)return;const top=el.getBoundingClientRect().top,d=top-state.anchor.top;if(Math.abs(d)>2&&Math.abs(d)<1200)scrollBy(0,d);state.anchor={symbol:state.anchor.symbol,top:el.getBoundingClientRect().top}})
}
...
function stability(){
 ...
 const mo=new MutationObserver(muts=>{let relevant=false;for(const m of muts){const x=m.target?.nodeType===1?m.target:m.target?.parentElement;if(x?.closest?.('#results,.top-grid,.hero')){relevant=true;break}for(const n of m.addedNodes||[]){if(n.nodeType===1&&(n.matches?.('#results,.top-grid,.hero,.card,.pick')||n.querySelector?.('.card,.pick'))){relevant=true;break}}if(relevant)break}if(relevant){queueInject();restoreAnchor()}});mo.observe(document.body,{childList:true,subtree:true});captureAnchor();installScrollGuard()
}
