/* DeepRise V15.5 — balanced entry gate + whale-flow confluence.
   Whale flow is a capped supporting/conflict factor; it never creates a trade by itself. */
(()=>{'use strict';
if(window.DeepRiseEntryRank?.version==='15.5')return;
...
function whaleFlow(x,side){try{return window.DeepRiseWhaleFlowV155?.support?.(x,side)||{available:false,supportive:false,conflict:false,score:0,stage:'NONE',eta:'—'}}catch(e){return{available:false,supportive:false,conflict:false,score:0,stage:'NONE',eta:'—'}}}
function hiddenGate(x,side){
 const p=x?.proSignal||{},confidence=n(p.confidence,x?.score),rank=n(p.rank,confidence),rsi=n(x?.rsi,50),rawExpected=Number(p.expectedMovePct),hasExpected=Number.isFinite(rawExpected)&&Math.abs(rawExpected)>.01,expected=hasExpected?rawExpected:0,detailed=!!p.detailed,riskCount=oppositeRiskCount(x,side),flow=whaleFlow(x,side),reasons=[];
 const checks={
 ...
 if(!checks.whaleConflict)reasons.push('strong opposing whale-flow evidence');
 return{ok:Object.values(checks).every(Boolean),checks,reasons,confidence,rank,riskCount,detailed,hasExpected,whaleFlow:flow};
}
function quality(x){
 const side=rawSide(x),long=side==='LONG',short=side==='SHORT';
 if(!x||(!long&&!short))return{side:'WAIT',count:0,state:'WAIT',items:[],entryReady:false,entryMode:'NONE',hardAligned:false,hidden:{ok:false,reasons:['no directional edge'],whaleFlow:{available:false}}};
 ...
 const count=items.filter(v=>v[1]).length,hardAligned=!!(items[0][1]&&items[1][1]),hidden=hiddenGate(x,side),missing=items.filter(v=>!v[1]).map(v=>v[0]),flow=hidden.whaleFlow||{};
 ...
 const flowAssist=flow.supportive===true&&n(flow.score)>=80;
 const confidenceFloor=flowAssist?78:80,rankFloor=flowAssist?74:76;
 const fastTrack=onlySoftMissing&&hardAligned&&hidden.checks.forecastAlignment&&hidden.checks.expectedDirection&&hidden.checks.notOverextended&&hidden.checks.volatility&&hidden.checks.rewardRisk&&hidden.checks.liquidity&&hidden.checks.whaleConflict&&hidden.riskCount===0&&confidence>=confidenceFloor&&rank>=rankFloor&&n(x.rr)>=2&&n(x.atrPct)<=5.5&&softFloor&&companionConfirm;
 ...
 else if(count>=5&&hardAligned&&confidence>=70&&rank>=64&&hidden.riskCount<2&&!flow.conflict){state='CONFIRMED'}
 else if(count>=4&&hardAligned&&hidden.riskCount<2&&!flow.conflict){state='FORMING'}
 return{side,count,state,items,missing,entryReady:state==='READY',entryMode,hardAligned,confidence,rank,hidden,whaleFlow:flow};
}
...
function applyGate(){
 installQuality();
 for(const x of data()){
  const q=quality(x);x.v13Quality=q;x.entryDecision={state:q.state,side:q.side,count:q.count,confidence:q.confidence||0,rank:q.rank||0,hidden:q.hidden,entryAllowed:q.entryReady,entryMode:q.entryMode,missing:q.missing,whaleFlow:q.whaleFlow,updated:Date.now()};
  if((q.side==='LONG'||q.side==='SHORT')&&q.state==='READY')x.direction=q.side;else x.direction='WAIT';
 }
}
function strength(x,f){const q=quality(x),p=n(x?.proSignal?.rank,x?.score),c=n(x?.proSignal?.confidence,x?.score),pump=n(x?.pump?.score),flow=q.whaleFlow||{};if(f==='pump')return pump*2+p*.35+q.count*2;if(f==='wait')return q.count*24+p+c*.2+(q.state==='CONFIRMED'?24:q.state==='FORMING'?14:0)+(flow.supportive?8:0)-(flow.conflict?18:0);const bonus=q.entryMode==='STRICT'?95:q.entryMode==='FAST_TRACK'?82:q.state==='CONFIRMED'?38:q.state==='FORMING'?12:0;return bonus+p+c*.5+q.count*7+n(x?.volumeRatio)*2-(q.hidden?.riskCount||0)*12+(flow.supportive?12:0)-(flow.conflict?30:0)}
...
window.DeepRiseEntryRank={version:'15.5',quality,hiddenGate,whaleFlow,applyGate,applyFilter,strength,refresh};
